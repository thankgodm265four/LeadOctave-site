...
export default function Footer() {
  ...
  return (
    <footer className="bg-[#0a1628] relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 py-16">
          <div className="flex flex-col lg:flex-row items-center justify-between gap-8">
            <div className="lg:w-1/2">
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-[#C19A6B]/10 rounded-full mb-4">
                <Sparkles className="w-4 h-4 text-[#C19A6B]" />
                <span className="text-sm text-[#C19A6B] font-bold uppercase tracking-wider">Stay Updated</span>
              </div>
            ...
            <div className="lg:w-1/2 w-full">
              <form className="flex flex-col sm:flex-row gap-4">
                <input
                  ...
                  className="flex-1 px-6 py-4 bg-white/5 border border-white/10 rounded-xl text-white placeholder-gray-500 focus:border-[#C19A6B] focus:ring-2 focus:ring-[#C19A6B]/20 outline-none transition-all"
                />
                <button
                  type="submit"
                  className="group px-8 py-4 bg-gradient-to-r from-[#C19A6B] to-[#A07855] text-white font-bold rounded-xl transition-all duration-300 shadow-lg hover:shadow-[#C19A6B]/30 hover:scale-105 flex items-center gap-2 whitespace-nowrap"
                >
                  Subscribe
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-12">
          ...
          <div>
            <h4 className="text-white font-bold mb-6 flex items-center gap-2">
              Services
              <ArrowUpRight className="w-4 h-4 text-[#C19A6B]" />
            </h4>
            <ul className="space-y-3">
              {footerLinks.services.map((link, index) => (
                <li key={index}>
                  <a
                    href={link.href}
                    className="text-gray-400 hover:text-[#C19A6B] transition-colors text-sm underline-animation"
                  >
                    {link.label}
                  </a>
                </li>
          ...
          <div className="col-span-2 md:col-span-1">
            ...
            <ul className="space-y-4">
              ...
              <li>
                <a href="tel:09061679205" className="flex items-start gap-3 text-gray-400 hover:text-[#C19A6B] transition-colors text-sm">
                  <Phone className="w-5 h-5 flex-shrink-0 mt-0.5" />
                  <span>09061679205</span>
                </a>
              </li>
              <li>
        ...
        <div className="max-w-7xl mx-auto px-6 py-6">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="flex items-center gap-6">
              {footerLinks.legal.map((link, index) => (
                <a
                  key={index}
                  href={link.href}
                  className="text-sm text-gray-500 hover:text-[#C19A6B] transition-colors"
                >
                  {link.label}
                </a>
              ))}
            ...
            <p className="text-sm text-gray-500 flex items-center gap-2">
              Built with <span className="text-[#C19A6B]">♥</span> for leaders
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-[#C19A6B] via-transparent to-[#C19A6B] opacity-30"></div>
    </footer>
  );
}
